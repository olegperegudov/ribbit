...
fn vocab_section(vocab: &HashMap<String, Vec<String>>) -> String {
    ...
    s.push_str("Словарь обязателен и важнее правила про англицизмы: термины и названия из него пиши ровно так, как в левой колонке. Подставляй правильное написание не только при точном совпадении с вариантом справа, но и при очевидных искажениях по смыслу (если по контексту речь явно про слово из словаря — заменяй, даже если форма отличается). Не подставляй, когда смысл не подходит.");
    s
}
...
Allrosa, Allros, AllRoss, алроса, алросе.";
    ...
    fn system_prompt_snapshot_with_vocab() {
        ...
        assert!(p.contains("Словарь обязателен"));
    }
